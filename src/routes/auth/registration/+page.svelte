<script>
	...
	let credential = { email: '', password: '', confirmPassword: '' };
	let errorMessage = '';
	function signup() {
		...
		if (
			...
			createUserWithEmailAndPassword(auth, credential.email, credential.password).then(r => {
	...
	function checkEmailAndPassword() {
		...
		if (credential.password !== credential.confirmPassword)
			errorMessage += `пароль и подтверждение пароля должны быть равны<br/>`;
	}
</script>
...
<div class="d-flex justify-content-center align-items-center" style="height: 100vh;">
	<div class="bg-light text-dark p-3 rounded" style="min-width:23em;">
		<div class="mb-2 text-center">
			<b class="text-uppercase">Регистрация на сайте</b>
		</div>
		...
		<div class="mt-4">
			...
			<div class="input-group mb-4">
				<input class="form-control" type="password" placeholder="подтверждение пароля" bind:value={credential.confirmPassword} />
			</div>
			<div class="d-flex gap-1 justify-content-between align-items-center">
				<button class="btn btn-dark text-light" on:click={() => signup()}>Зарегистрироваться</button>
